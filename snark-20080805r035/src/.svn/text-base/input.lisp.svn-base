...
(defun can-be-sort-name (x &optional action)
  ...
  ;; if a sort name ends in a digit or number sign as in child1 or card#,
  ;; it must be followed by a number sign in variable names as in ?child1#, ?child1#13, ?card##, ?card##13
  (or (top-sort-name? x)
      (and (can-be-name1 x)
           (not (eq 'and x))
           (not (eq 'or x))
...
(defun input-quantifier-variable (var-spec)
  ...
  (cond
   ...
   ((and (evenp (length var-spec)) (top-sort-name? (second var-spec)))
    (setf var-spec (cons (first var-spec) (cddr var-spec))))
